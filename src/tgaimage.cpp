...
TGAImage::TGAImage() : data(NULL), width(0), height(0), bytespp(0), last() {}

TGAImage::TGAImage(int w, int h, int bpp) : data(NULL), width(w), height(h), bytespp(bpp), last() {
...
TGAColor TGAImage::get(int x, int y) {
    if (!data || x<0 || y<0 || x>=width || y>=height) {
        return TGAColor(255, 0, 0);
    }
    last = TGAColor(data+(x+y*width)*bytespp, bytespp);
    return last;
}
